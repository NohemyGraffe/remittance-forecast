...
def plot_tx(data, fore):
    with st.expander("📈 Show Transactions Forecast"):
...
def plot_value(data, fore):
    ...
    with st.expander("💰 Show Value Forecast (MXN)"):
        fig, ax1 = plt.subplots(figsize=(11,5))
        ...
        # Optional download
        st.download_button("📥 Download forecast CSV", data=fore.to_csv(index=False), file_name="weekly_forecast.csv")
