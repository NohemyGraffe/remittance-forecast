...
# Series IDs (these are safe to keep as constants)
...
def fetch_weekly_from_banxico(
    ...
    # 1) explicit arg
    # 2) env BANXICO_TOKEN
    # 3) env DEFAULT_BANXICO_TOKEN
    token = token or os.getenv("BANXICO_TOKEN") or os.getenv("DEFAULT_BANXICO_TOKEN")
    if not token:
        raise RuntimeError("No Banxico token provided. Set BANXICO_TOKEN (or DEFAULT_BANXICO_TOKEN) env var, "
                           "or pass token=... from Streamlit secrets.")
...
def main():
    ...
    ap.add_argument("--banxico-token", default=None, help="Banxico API token (else use env BANXICO_TOKEN/DEFAULT_BANXICO_TOKEN)")
    ...
    # Resolve token from CLI or environment
