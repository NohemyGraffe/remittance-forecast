...
def main():
    ...
    try:
        ...
        # --- add this to fetch_real_data.py ---
...
def fetch_weekly_from_banxico(
    ...
    if token is None:
        # Prefer env var; never hard-code token in repo
        token = os.getenv("DEFAULT_BANXICO_TOKEN")
        if not token:
            raise RuntimeError("BDEFAULT_BANXICO_TOKEN missing. Set it in env or Streamlit secrets.")
